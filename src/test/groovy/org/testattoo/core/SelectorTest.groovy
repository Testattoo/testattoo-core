...
class SelectorTest {
    @BeforeEach
    void before() {
        config.evaluator = Mockito.mock(Evaluator)
    ...
    void should_throw_an_error_if_single_selector_find_many_components() {
        when(config.evaluator.metaInfo("\$('expression')"))
    ...
    void should_throw_an_error_if_single_selector_return_not_expected_component_type() {
        ...
        when(config.evaluator.check('1', identifyingExpression(CustomElement_2))).thenReturn(true)
    ...
    void should_have_at_least_an_identifier_available_on_component() {
        ...
        try {
            ...
            assert e.message == 'Missing @Identifier annotation on type org.org.testattoo.core.SelectorTest$InvalidComponent'
        }

        config.componentTypes.add(BigButton)
        when(config.evaluator.metaInfo("\$('button')")).thenReturn([new MetaInfo(id: '2', node: 'BigButton')])
        when(config.evaluator.check('2', identifyingExpression(BigButton))).thenReturn(true)
    ...
    void should_use_$$_as_a_multi_selector() {
        when(config.evaluator.metaInfo("\$('.btn')"))
            .thenReturn([new MetaInfo(id: '1', node: 'node'),
                         new MetaInfo(id: '2', node: 'node'),
                         new MetaInfo(id: '3', node: 'node')])
